...
import com.example.upnews.api.RetrofitInstance
...
class HeadlinesFragment : Fragment(R.layout.fragment_headlines) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        retryButton.setOnClickListener {
            // Check if we're currently on the HeadlinesFragment
            val currentFragment = findNavController().currentDestination?.id

            if (currentFragment == R.id.headlinesFragment) {
                // Reload headlines if we're already on HeadlinesFragment
                newsViewModel.getHeadlines("us")
            } else {
                // Navigate back to HeadlinesFragment if we're not already there
                findNavController().navigate(R.id.headlinesFragment)
            }
        }
